...
Rectangle {
  ...
  MouseArea {
    ...
    focus: true;
    Keys.onEscapePressed: Screensaver.Controller.open = false;
  }
  ...
  FrameAnimation {
    ...
    running: root.width > logo.width && root.height > logo.height;
    ...
    onTriggered: {
      if (logo.x <= 0 || logo.x >= root.width - logo.width) {
        logo.x = Math.max(0, Math.min(logo.x, root.width - logo.width));
        xSpeed *= -1;
        logo.changeColor();
      }
      logo.x += xSpeed * frameTime;

      if (logo.y <= 0 || logo.y >= root.height - logo.height) {
        logo.y = Math.max(0, Math.min(logo.y, root.height - logo.height));
        ySpeed *= -1;
        logo.changeColor();
      }
