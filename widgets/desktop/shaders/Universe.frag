...
// SOURCE: https://www.shadertoy.com/view/lscczl
// Changes made:
//   - Port to qt
//   - Make background transparent
//   - Remove simple mode
//   - Remove glow effect that reacts to music
...
void main() {
  ...
  col *= 1. - dot(uv, uv);
  float t_mod = mod(iTime, 230.0);
  col *= S(0., 20., t_mod) * S(224., 200., t_mod);

  // Brighter colors get a higher value, darker ones get lower a value
  float luminance = max(col.r, max(col.g, col.b));

  fragColor = vec4(col, luminance) * ubuf.qt_Opacity;
}
